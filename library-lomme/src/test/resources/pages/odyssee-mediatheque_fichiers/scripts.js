//modules -> profil, rubriquePage, carrouselCentralDisplay
window.onerror = function(desc, page, line, chr) {
	/*
	 * alert('JavaScript error occurred! \n' +'\nError description: \t'+desc
	 * +'\nPage address: \t'+page +'\nLine number: \t'+line this.blur(); );
	 */
}
//
$(function() {
	...
	$('#slide-holder ul').hover(function() {
		window.clearTimeout(slider.al);
	}, function() {
		slider.al = window.setTimeout('slider.auto();', slider.at);
	});

	// window.clearTimeout(slider.ar);
	...
	$(window).load(function() {
		// $.each(css_ims,function(){(new
		// Image()).src=_siteRoot+'css/images/'+this;});
		/*
		 * $.each(css_cims,function(){ var css_im=this;
...
var slider = {
	...
	init : function() {
		...
		for ( var i = 0; i < slider.data.length; i++) {
			if ( $('#' + slider.data[i].id).length ) {
				d[i] = slider.data[i];
			}
		}
		
		slider.data = d;
		slider.num = slider.data.length;
		
		if ( slider.num == 0) { return; }
	...
	ticker : function(el, text, pos, unique) {
		...
		/*
		 * if(pos==text.length) $(el).html(text); else
