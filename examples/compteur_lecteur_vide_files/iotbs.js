...
/*
if(document.getElementById('screen-switcher')!==null){
    var screenSwitcher = new bodySwitcher('screen-switcher', 'Taille du texte ');

    //add a new class option ('classname', 'label')
    screenSwitcher.defineClass('default', '-');
    screenSwitcher.defineClass('fontsize_medium', 'Moyen');
    screenSwitcher.defineClass('fontsize_big', 'Grand');
}

if(document.getElementById('screen-switcher2')!==null){
    var screenSwitcher2 = new bodySwitcher('screen-switcher2', 'Couleurs ');

    //add a new class option ('classname', 'label')
    screenSwitcher2.defineClass('default', '-');
    screenSwitcher2.defineClass('colors_blackwhite', 'Blanc sur noir');
    screenSwitcher2.defineClass('colors_whiteblack', 'Noir sur blanc');
    screenSwitcher2.defineClass('colors_blackyellow', 'Bleu sur jaune');
}

if(document.getElementById('screen-switcher3')!==null){
    var screenSwitcher3 = new bodySwitcher('screen-switcher3', 'Police ');

    //add a new class option ('classname', 'label')
    screenSwitcher3.defineClass('default', '-');
    screenSwitcher3.defineClass('fontfamily_courier', 'Courier');
    screenSwitcher3.defineClass('fontfamily_arial', 'Arial');
    screenSwitcher3.defineClass('fontfamily_tahoma', 'Tahoma');
	screenSwitcher3.defineClass('fontfamily_trebuchet_ms', 'Trebuchet MS');
}
*/
/*****************************************************************************
*****************************************************************************/
...
function switchManager()
{
	...
	if(this.cookie != null)
	{
		...
		// GJ-20071030 : Comme on fixe les classes côté serveur, on a pas besoin de refixer les classes en javascript (on crée des doublons sinon)
...
function bodySwitcher(divid, label)
{
	...
	this.attrs = { 'for' : 'select-' + divid , 'class' : 'visible' };
	...
	this.select.onchange = function()
	{
		...
		switcher.body.className = switcher.string;  // GJ-20071030 : Comme on fixe les classes côté serveur, on a pas besoin de switcher.initial (on provoque des doublons de classe sinon)
